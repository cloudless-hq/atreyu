<script>
  ...
  function toggle (key) {
    localStorage['ayu_' + key] = !JSON.parse(localStorage['ayu_' + key])
    location.reload()
  }

  let preload = localStorage.ayu_preload === undefined ? true : localStorage.ayu_preload
  let updating = localStorage.ayu_updating === undefined ? true : localStorage.ayu_updating
  let verbose = localStorage.ayu_verbose === undefined ? true : JSON.parse(localStorage.ayu_verbose)
  let logging = localStorage.ayu_logging === undefined ? true : JSON.parse(localStorage.ayu_logging)
  let batching = localStorage.ayu_batching === undefined ? true : JSON.parse(localStorage.ayu_batching)
</script>
...
<Menu let:open let:versionTooltip>
  ...
  <div
    ...
    <div class="py-1" role="none">
      <div class="text-gray-700 group flex items-center px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-0" on:click={() => { toggle('preloading') }}>
      ...
      <div class="text-gray-700 group flex items-center px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-1" on:click={() => { toggle('logging') }}>
      ...
      {#if logging}
        <div class="text-gray-700 group flex items-center px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-1" on:click={() => { toggle('verbose') }}>
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="mr-3 h-5 w-5 text-gray-400 group-hover:text-gray-500">
      ...
      <div class="text-gray-700 group flex items-center px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-1" on:click={() => { toggle('updating') }}>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="mr-3 h-5 w-5 text-gray-400 group-hover:text-gray-500">
          <path stroke-linecap="round" stroke-linejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182m0-4.991v4.99" />
        </svg>

        {updating ? 'Disable' : 'Enable'} Auto Updating
      </div>

      <div class="text-gray-700 group flex items-center px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-1" on:click={() => { toggle('batching') }}>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="mr-3 h-5 w-5 text-gray-400 group-hover:text-gray-500">
