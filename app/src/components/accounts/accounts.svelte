...
{#if accounts.length}
  <div class="userswitcher p-24">
    <h1 class="text-3xl mb-4 font-bold leading-tight tracking-tight text-gray-900 text-left">My Accounts on this device</h1>

    <ul class="mt-2 grid grid-cols-1 gap-30 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
      {#each accounts as account}
        <li class="w-max group relative col-span-1 flex flex-col transition-shadow rounded-lg bg-white text-center shadow hover:shadow-lg">
          ...
          <div class="flex flex-1 flex-col p-8">
            ...
            <button on:click={() => { account.isLoggedIn ? window.location.href = ((new URLSearchParams(location.search)).get('continue') || '/') : doLoginUser({ sessionId: account.sessionId, email: account.username, org: account.org }) }} class="relative cursor-pointer mx-auto h-32 w-32 flex-shrink-0 rounded-full" style="height: 65px; width: 65px; background: #e5e5e5; fill: #858585; padding: 16px;">
