<script>
  ...
  $: sessionName = `${isIncognito ? 'Incognito session ' : ''}${hasBrands ? navigator?.userAgentData?.brands?.[navigator?.userAgentData?.brands?.length - 1].brand : 'unknown browser'} on ${navigator?.userAgentData?.platform || 'unknown platform'}`
  ...
  <div class="login" class:hidden={!!userData}>
    <form class="login-form" action={formTarget} method="post" name="loginForm">
      ...
      <div class="login-input">
        ...
        <input name="email" type="email" class="input" placeholder="Email" required value="{userData?.email || 'dev_user@localhost'}" />
      </div>

      {#if userData?.sessionId}
        <input name="sessionId" value="{userData.sessionId}" />
      {/if}
      ...
      <div class="login-input">
        ...
        <input id="org" name="org" class="input" placeholder="Organization" value="{userData?.org || ''}">
      </div>
