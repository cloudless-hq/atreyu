    ...
    return new Proxy(subObj, {
      ...
      apply(target, thisArg, args) {
        ...
        if (cleanKey === "_loadRef") {
          const ref3 = args[0];
          if (ref3?.length > 0) {
            return getJsonPath(thisArg, ref3);
          } else {
            return thisArg;
          }
        }
...
function makeDataStore({ source, batched = true, maxSize, collectRatio, maxRetries, cache, onChange = () => {
  ...
  let model = index({
    ...
    // },
    errorSelector: function(path, error3, c) {
      if (errorSelector2) {
        errorSelector2(path, error3, c);
      } else {
        console.error(path, error3, c);
      }
      return error3;
    }
  }).treatErrorsAsValues();
  ...
  const makeAyuProxy = (id, subModel) => makeProxy({
    ...
    get: (path, subVal, delim, id2) => {
      ...
      const pathString2 = path.join(".") + (boxKey ? `.${boxKey}` : "");
      if (!deps[id2]) {
      ...
      let newProm;
      if ((falcorCacheVal === void 0 || latestTick !== lastUpdt.get(pathString2)) && delim !== "$refKey") {
        ...
        newProm = adjustedModel.getValue(path).then((val) => {
          if (typeof val === "undefined" || val?.$type === "atom" && val?.value === void 0) {
            ...
            if (delim === "$key") {
              ...
              placeholders.set(pathString2, true);
            }
          } else {
            if (delim === "$ref") {
              if (val.$type === "ref") {
            ...
            if (delim === "$key") {
              if (key) {
                ...
                if (!previousViewKeys[curViewKey]) {
                  ...
                  keys.delete(pathString2);
                }
              } else if (cacheVal === _undefined || placeholders.has(pathString2)) {
                keys.delete(pathString2);
              }
              placeholders.delete(pathString2);
            }
      ...
      if (delim === "$key" || delim === "$refKey") {
        if (falcorCacheVal === void 0) {
          if (duplicates.firstIds) {
            ...
            if (firstIdPath && firstIdPath !== pathString2) {
              placeholders.set(pathString2, { fresh: true });
            }
          }
        }
        if (cacheVal === _undefined || falcorCacheVal === _undefined) {
          const placeholder = placeholders.get(pathString2);
          if (placeholder?.fresh || !placeholder) {
            placeholders.set(pathString2, { fresh: false });
            keys.delete(pathString2);
          }
        }
        const pathOverride = keys.get(pathString2);
        if (cacheVal && cacheVal !== _undefined) {
          if (delim === "$refKey") {
          ...
          const isExpired3 = currentViewValueOverride ? !(currentViewValueOverride.latestTick + 6 > latestTick) : null;
          ...
          if (falcorCacheVal !== void 0 && falcorCacheVal !== _undefined) {
            ...
            duplicates.firstIds.set(`${cacheVal}`, pathString2);
          }
          const noOverrides = !valueOverrides && !pathOverride;
          if (noOverrides) {
            return cacheVal;
          }
          if (currentViewValueOverride && !isExpired3 && !pathOverride) {
            return currentViewValueOverride.key;
          }
          if (latestValueOverride && !pathOverride && !placeholders.has(pathString2)) {
            return latestValueOverride.key;
          }
          if (!currentViewValueOverride && !placeholders.has(pathString2)) {
            if (!valueOverrides) {
        ...
        key = `first_seen_${pathString2}_${latestTick} `;
        keys.set(pathString2, key);
        return key;
      }
  ...
  function update() {
    if (subscribers.size > 0) {
      ...
      subscribers.forEach(([run, invalidate2, subscriptionProxy, id]) => {
        let changed = false;
        if (!deps[id]) {
          changed = true;
        } else {
  ...
  let subscrCounter = 0;
  function subscribe2(run, invalidate2, subModel) {
  ...
  const dataStore = {
    deref: (paths) => {
      return paths.map((path) => {
        ...
        return {
          subscribe: (run, invalidate2) => {
            return subscribe2(run, invalidate2, subModel);
          },
          set: () => {
          },
          falcor: subModel
